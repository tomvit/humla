...
Slide = function(element, footer, section, number) {
  ...
  this.__defineGetter__('description', function() {
    return {
      title: this.title,
      "slide-url": humla.baseURI + this.url,
...
Section = function(element, parent) {
  ...
  this.__defineGetter__('contents', function() {
    ...
    var slides = []
    for (var i = 0; i < this.slides.length; i++)
      if (this.slides[i].classNameOriginal == "slide")
        slides.push({
          "title": this.slides[i].title,
          "slide-url": humla.baseURI + this.slides[i].url
        })

    var cont = {
      title: parent != null ? this.name : window.document.title,
