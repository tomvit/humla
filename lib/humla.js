...
var humla = {
  ...
  findSlide: function(number) {
    for (var i = 0; i < this.slides.length; i++)
      if (this.slides[i].url == '#/' + number)
  ...
  load: function() {
    ...
    // function to read main and supplementary configuration
...
var MessageBox = function(message) {
  ...
  // this.prompt = function(message, fields) {
  //   if (!this.disabled) {
  //     if (this.div) {
  //         humla.utils.documentBody.removeChild(this.div);
  //   	    this.div = null;
  ...
  //     this.div.innerHTML = "<span class='close' onclick='humla.msgbox.dismiss();'></span>";
  //     this.div.innerHTML = "<p>" + message + "</p>" + this.div.innerHTML;
  // 
  //     for (var inx=0; inx < fields.length; inx++) {
  //         if (fields[inx].type === "button") {
  //           this.div.innerHTML += "<button type='button'>" + fields[inx].label + "</button>"
  //         }
  //     }
  // 
  // 
